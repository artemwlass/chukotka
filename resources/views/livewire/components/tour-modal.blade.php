<div>
    <div class="modal book-tour fade" id="tourModal" tabindex="-1" aria-labelledby="tourModalLabel" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content align-items-start">
                ...
                <h2>{{__('Заявка на подбор')}} <br class="d-sm-none"> {{__('корпоративного тура')}}</h2>
                ...
                <div class="form_checkbox d-flex align-items-start">
                    <input type="checkbox" name="" id="check-3"  wire:model="agree">
                    <label for="check-3" class="icon">
                ...
                <button class="btn_orange" wire:click="store">{{__('Оставить заявку')}}</button>
            </div>
        </div>
    </div>
    @script
    <script>
        $wire.on('open-thank-modal', () => {
            ...
            var tourModal = bootstrap.Modal.getInstance(document.getElementById('tourModal'));
            tourModal.hide();
            thankModal.show();
